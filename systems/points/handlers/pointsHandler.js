...
async function logPointOperation(client, operation) {
    try {
        ...
        if (!systemConfig?.logChannelId) return;

        const logChannel = client.channels.cache.get(systemConfig.logChannelId);
        if (!logChannel) return;

        const embed = {
            title: 'Point Operation Log',
            color: operation.action === 'ADD' ? 0x00ff00 : operation.action === 'REMOVE' ? 0xff9900 : 0xff0000,
            fields: [
                { name: 'Action', value: operation.action, inline: true },
                { name: 'User', value: `<@${operation.userId}>`, inline: true },
                { name: 'Points', value: operation.points.toString(), inline: true },
                { name: 'Admin', value: operation.adminId === 'SYSTEM' ? 'SYSTEM' : `<@${operation.adminId}>`, inline: true }
            ],
            timestamp: new Date()
        };
